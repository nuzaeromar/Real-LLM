...
import numpy as np
from transformers import BertConfig, BertTokenizerFast, BertForMaskedLM, RobertaConfig, RobertaTokenizerFast, RobertaForMaskedLM
import torch
...
class BERTAttacker(ClassificationAttacker):
    ...
    def __init__(self, 
            ...
            mlm_path : str = 'FacebookAI/roberta-base',
            k : int = 100,
            use_bpe : bool = True,
            sim_mat : Union[None, bool, WordSubstitute] = None,
            threshold_pred_score : float = 0.03,
        ...
        self.sbert_model = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2") #ADDED
    ...
    def attack(self, victim: Classifier, sentence, goal: ClassifierGoal):
        ...
        # original label
        inputs = tokenizer(feature.seq, add_special_tokens=True, max_length=max_length, truncation=True, return_attention_mask=True)
        input_ids = torch.tensor(inputs["input_ids"])
        # token_type_ids = torch.tensor(inputs.get("token_type_ids", [0] * len(ids)))
        ...
        # Build subword sequence with special tokens
        cls_tok, sep_tok, mask_tok = tokenizer.cls_token, tokenizer.sep_token, tokenizer.mask_token
        
        sub_words = [cls_tok] + sub_words[:max_length - 2] + [sep_tok]
        # sub_words = [tokenizer.bos_token] + sub_words[:2] + sub_words[2:max_length - 2] + [tokenizer.eos_token]
        ...
        for top_index in list_of_index:
            ...
            for substitute in substitutes:               
                ...
                bpe_tokens = tokenizer.tokenize(temp_text)
                temp_text = tokenizer.convert_tokens_to_string(bpe_tokens)
                
                inputs = tokenizer.encode_plus(temp_text, None, add_special_tokens=True, max_length=max_length, truncation=True)
                # inputs = tokenizer(temp_text, add_special_tokens=True, max_length=max_length, truncation=True, return_attention_mask=True)
                ...
                if count == 5 and not anthro_done:
                    ...
                    for alt in alt3:
                        ...
                        if alt == tgt_word or alt in self.filter_words:
                            continue
                        temp_replace = final_words.copy()   
        ...
        final_tokens = tokenizer.tokenize(joined)
        feature.final_adverse = tokenizer.convert_tokens_to_string(final_tokens)
        feature.success = 0
        ...
        if self.label_words and feature.label in self.label_words:
            ...
            #         bpe_tokens = tokenizer.tokenize(temp_text)
            #         cand_text   = tokenizer.convert_tokens_to_string(bpe_tokens)
            ...
            for w in pool:
                for gap in gaps[:num_to_try]:
                    ...
                    tokens.insert(gap, tokenizer.mask_token)
    ...
    def get_bpe_substitues(self, substitutes, tokenizer, mlm_model):
        ...
        substitutes = substitutes[0:15, 0:7] # maximum BPE candidates
        ...
        all_substitutes = all_substitutes[:30].to(self.device)
