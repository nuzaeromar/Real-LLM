...
import numpy as np
from transformers import BertConfig, BertTokenizerFast, BertForMaskedLM, AlbertForMaskedLM, AlbertTokenizerFast, AlbertConfig
import torch
...
class BERTAttacker(ClassificationAttacker):
    ...
    def __init__(self, 
        ...
        self.tokenizer_mlm = AlbertTokenizerFast.from_pretrained(mlm_path, do_lower_case=True)
        if device is not None:
            self.device = device
        else:
            self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        config_atk = AlbertConfig.from_pretrained(mlm_path)
        self.mlm_model = AlbertForMaskedLM.from_pretrained(mlm_path, config=config_atk).to(self.device)
        self.k = k
    ...
    def get_bpe_substitues(self, substitutes, tokenizer, mlm_model):
        ...
        all_substitutes = all_substitutes[:36].to(self.device)
